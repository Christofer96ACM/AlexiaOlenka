...
{
    public partial class FacturaAnticipo : Page
    {
        ...
        protected void gdvinv1_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else if (e.Parameters.Contains("SRCH"))
            {
                ...
                var obrd = new BRDocument();
                //Session["inv1"] = obrd.Get_OSCSP_SRCH_DPI1(obep);

                var list = obrd.Get_OSCSP_SRCH_DPI1(obep);
                list.ForEach(item => {
                    if (item.TreeType == "S")
                    {
        ...
        private string Save()
        {
            ...
            if (string.IsNullOrWhiteSpace(_err))
            {
                ...
                else
                {
                    ...
                    obj.DpmPrcnt = Convert.ToDecimal(alphanumeric(txtDiscPrcntTotal.Value.ToString()));
                    ...
                    using (var obrd = new BRDocument())
                    {
                            ...
                            //}
                            //else
                                obrd.SaveDownPayment(obj, ((BEParameters)Session["InitPar"]).objSapSbo);
                    }
        ...
        private string Cancel()
        {
            ...
            if (string.IsNullOrWhiteSpace(_err))
            {
                ...
                using (var obrd = new BRDocument())
                {
                    obrd.CancelDownInvoice(obj, ((BEParameters)Session["InitPar"]).objSapSbo);
                }
        ...
        private string Imprimir()
        {
            ...
            obj.LicTradNum = bteLicTradNum.Text.Trim();
            obj.NumAtCard = txtNumAtCard.Text;
            obj.formatoImpresion = "FORMATOFACTURAANTICIPO";
        ...
        protected void gdvOinvSearch_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            var obrd = new BRDocument();
            var olst = obrd.Get_OSCSP_SRCH_ODPI(obj);
            Session["oinv"] = olst;
