    ...
    public partial class DeliveryNote : Page
    {
        ...
        protected void gdvdln1_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else if (e.Parameters.Contains("SRCH"))
            {
                ...
                var obrd = new BRDocument();
                var olist = obrd.Get_OSCSP_SRCH_DLN1(obep);
                olist.ForEach(item => {
                    if (item.TreeType == "S")
                    {
                        item.Father = item.ItemCode;
                        for (var i = item.LineNum; i < olist.Count && olist[i].TreeType == "I"; i++)
                        {
                            olist[i].Father = item.ItemCode;
                        }
                    }
                ...
                olist.ForEach(item => {
                    ...
                    {
                        seriesUsadas = obrd.GET_SERIES_USADAS(parametros);
                ...
                if (olist.Count != 0)
                {
                    olist.ForEach(item =>
                    {
                        ...
                        olst.ForEach(a =>
                        {
                            ...
                            line2.DetalleServicio = a.DetalleServicio;
            ...
            else if (e.Parameters.Contains("ADDPRODUCT"))
            {
                ...
                if (listamateriales.Count == 0)
                {
                    articulo.ForEach(item =>
                    {
                        ...
                        line.DetalleServicio = item.DetalleServicio;
                        ((List<BEDocumentLine>)Session["dln1"]).Add(line);
                        Set_LineNum();
                    });
                }
                else
                {
                    articulo.ForEach(i =>
                    {
                        ...
                        line.DetalleServicio = i.DetalleServicio;
                ...
                if (!string.IsNullOrWhiteSpace(item.BatchNum))
                {
                    ...
                    _oibt.ForEach(_item =>
                    {
                        ...
                        if (_quan <= 0)
                        {
                            ...
                            //_item.Quantity = Convert.ToDouble(_item.Quantity) - qtemp;
                            dobreak = true;
                        }
                        else
                        {
        ...
        private string Save()
        {
            ...
            if (string.IsNullOrWhiteSpace(_err))
            {
                ...
                obj.U_DXP_FE_MOTIVO_TRAS = cbbMotTras.Value.ToString();
                obj.U_DXP_FE_MODALIDTRAS = cbbModTras.Value.ToString();
                ...
                using (var obrd = new BRDocument())
                {
                    if (band)
                    {
                        obrd.SaveDraft(obj, ((BEParameters)Session["InitPar"]).objSapSbo, 15);
                    }
                    else
                        obrd.SaveDeliveryNote(obj, ((BEParameters)Session["InitPar"]).objSapSbo);
                }
        ...
        private string Imprimir()
        {
            ...
            obj.LicTradNum = bteLicTradNum.Text.Trim();
            obj.NumAtCard = txtNumAtCard.Text;
            obj.formatoImpresion = "FORMATOGUIA";
        ...
        protected void gdvodlnSearch_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            var obrd = new BRDocument();
            var olst = obrd.Get_OSCSP_SRCH_ODLN(obj);
            Session["odln"] = olst;
