...
namespace OneCommerce.Sales
{
    public partial class ARInvoice : Page
    {
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public static List<BEParameters2> GET_ARTIC(string socied)
        {
            var obep = new BEParameters2()
            {
                Socied = socied
            };
            var obrd = new BRDocument();
            var olst = obrd.GET_ARTICULOS_V(obep);
            return (olst);
        }
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        ...
        private string Save()
        {
            ...
            if (string.IsNullOrWhiteSpace(_err))
            {
                ...
                //{
                    ...
                    obj.Socied = obec.Socied;
                    obj.U_DXP_SEDE = obec.U_DXP_SEDE;
                    ...
                    obj.Reserve = "N";
                    obj.DIRECCIONIP = obec.DIRECCIONIP;
                    obj.USUARIOMKT = obec.USUARIOMKT;
                    obj.PASSWORDMKT = obec.PASSWORDMKT;
                    obj.IPONU = obec.IPONU;
                    obj.USUARIONU = obec.USUARIONU;
                    obj.PASSWORDONU = obec.PASSWORDONU;
                    //para auditoria
                    obj.U_DXP_USUARIO_MOD = obec.UserName;
                    obj.U_U_DXP_FECHA = DateTime.Today;
                    ...
                    _err = !string.IsNullOrWhiteSpace(obj.Msg) ? obj.Msg : string.Format("OKS#{0}#{1}#{2}", obj.DocEntry, obj.DocNum,obj.MsjMikrot);
                //}
            }
