...
{
    public partial class Oferta : Page
    {
        ...
        protected void gdvrdr1_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else if (e.Parameters.Contains("SRCH"))
            {
                ...
                var obrd = new BRDocument();
                //Session["rdr1"] = obrd.Get_OSCSP_SRCH_QUT1_OFE(obep);
                var list = obrd.Get_OSCSP_SRCH_QUT1_OFE(obep);
                list.ForEach(item => {
                    if (item.TreeType == "S")
                    {
            ...
            else if (e.Parameters.Contains("OQUT"))
            {
                ...
                list.ForEach(item => {
                    if(item.TreeType == "S")
                    {
                        for(var i = item.LineNum+1; i < list.Count && list[i].TreeType =="I"; i++)
                        {
                            if(list[i].TreeType == "I")
                            {
                                list[i].Father = item.ItemCode;
                            }
                        }
        ...
        private string Save()
        {
            ...
            if (string.IsNullOrWhiteSpace(_err))
            {
                ...
                using (var obrd = new BRDocument())
                {
                    ...
                    obrd.SaveOfertaVenta(obj, ((BEParameters)Session["InitPar"]).objSapSbo);
                }
        ...
        private string Modify()
        {
            ...
            if (string.IsNullOrWhiteSpace(_err))
            {
                ...
                using (var obrd = new BRDocument())
                {
                    obrd.ModifyOfferSale(obj, ((BEParameters)Session["InitPar"]).objSapSbo);
                }
        ...
        private string Cancel()
        {
            ...
            using (var obrd = new BRDocument())
            {
                obrd.CancelOfferSale(obj, ((BEParameters)Session["InitPar"]).objSapSbo);
            }
        ...
        private string Imprimir()
        {
            ...
            obj.NumAtCard = txtNumAtCard.Text;
            obj.formatoImpresion = "FORMATOOFERTAVENTA";
        ...
        protected void gdvOrdrSearch_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            var obrd = new BRDocument();
            var olst = obrd.Get_OSCSP_SRCH_OQUT_OFE(obj);
            Session["ordr"] = olst;
