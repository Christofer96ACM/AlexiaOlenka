...
namespace OneCommerce.Masters
{
    public partial class Suppliers : Page
    {
        ...
        protected void listdirecciones_Callback(object sender, CallbackEventArgsBase e)
        {
            ...
            //else if (e.Parameter.Contains("UPDT"))
            //{
            //    //var lista = "";
            //    var spl = e.Parameter.Split(':');

            //    var olist = ListaDirecciones;
            //    ListaDirecciones.ForEach(item =>
            //    {
            //        if(item.LineNum.ToString() == spl[1]) { 
            //            item.Address = textAddress.Text;
            //            item.Street = txtStreet.Text;
            //            item.Country = "PE";
            //            item.AdresType = rdbtntipofact.SelectedItem.Value.ToString();
            //            item.State = cbbState.Value.ToString();
            //            item.U_DXP_BIZ_COPR = cbbU_DXP_BIZ_COPR.Value.ToString();
            //            item.U_DXP_BIZ_PROV = cbbU_DXP_BIZ_COPR.Text;
            //            item.U_DXP_BIZ_CODI = cbbU_DXP_BIZ_CODI.Value.ToString();
            //            item.U_DXP_BIZ_DIST = cbbU_DXP_BIZ_CODI.Text;
            //        }
            //    });
            //    Session["dire"] = ListaDirecciones;
            //    //listdirecciones. = ((List<BEClientAddress>)Session["dire"]);
            //    //listdirecciones.DataBind();
            //}
        ...
        protected void gvcontacto_RowUpdating(object sender, ASPxDataUpdatingEventArgs e)
        {
            ...
            ((List<BEContactLines>)Session["crd1"])[index].UPDT = "SI";
            gvcontacto.CancelEdit();
            e.Cancel = true;
            gvcontacto.DataSource = ((List<BEContactLines>)Session["crd1"]);
            gvcontacto.DataBind();
        ...
        protected void gvDirecciones_RowDeleting(object sender, ASPxDataDeletingEventArgs e)
        {
            ...
            var _lnm = (i + 1);
            ((List<BEClientAddress>)Session["dire"]).RemoveAll(item => item.LineNum == _lnm);
            Set_LineNum();

            //gvcontacto.JSProperties["cpSubTotal"] = ((List<BEContactLines>)Session["inv1"]).Sum(item => Math.Round(item.LineTotal, 2));

            gvDirecciones.DataSource = ((List<BEContactLines>)Session["dire"]);
            gvDirecciones.DataBind();
        }
