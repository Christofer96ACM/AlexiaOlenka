...
{
    public partial class PedidoCompra : Page
    {
            ...
            if (e.Parameters.Contains("QK"))
            {
                ...
                olst.ForEach(item =>
                {
                    var line = new BEDocumentLine()
                    {
                        ...
                        WhsCode = getAlmacen(item.ItemCode),
                        U_StoDiBE = Convert.ToDecimal(item.U_StoDiBE),
                        U_BF_StoDiYo = Convert.ToDecimal(item.U_BF_StoDiYo),
                        BL050 = Convert.ToDecimal(item.BL050),
                        BL010 = Convert.ToDecimal(item.BL010),
                        U_U_BF_CompTot = Convert.ToDecimal(item.U_U_BF_CompTot),
                        U_BIZ_DREC = item.ItemName,
                        U_BIZ_FFAR = item.U_BIZ_FOFA,
                        U_BIZ_CANT = item.U_BIZ_CAPC,
                        U_BF_FECH_EXPE = DateTime.Today.ToShortDateString(),
                        U_BF_FECH_VENC = DateTime.Today.AddDays(90).ToShortDateString(),
